 ...
 */
contract RWAToken is Ownable, OFT {
  ...
  ) Ownable(msg.sender) OFT(_name, _symbol, _lzEndpoint) {
    require(
      _lzEndpoint != address(0),
      "RWAToken::constructor: LayerZero endpoint cannot be the zero address."
    );
    require(
      _mainChainId != 0,
      "RWAToken::constructor: Main chain ID cannot be zero."
    );
    require(
      _initialSupplyOnMainChain > 0,
      "RWAToken::constructor: Initial supply of the token on the main chain must be greater than zero."
    );
