...
contract LayerZeroBridge is Ownable, Pausable {
  ...
  mapping(IOFT => uint256) public supportedTokens;
  ...
  event DailyLimitUpdated(IOFT indexed token, uint256 newLimit, uint256 timestamp);

  /// @notice An event emitted when a token is transferred from one chain to another
  event BridgeTransfer(IOFT indexed token, address indexed sender, address indexed recipient, uint16 dstChainId, uint256 amount, uint256 timestamp);
   ...
   * @dev The owner of the contract cannot be the zero address
   */
  constructor(address[] memory _tokens, uint256[] memory _dailyLimits) Ownable(msg.sender) {
    for (uint256 i = 0; i < _tokens.length; i++) {
      IOFT token = IOFT(_tokens[i]);
      dailyLimit = _dailyLimits[i];

      require(token != IOFT(address(0)), "LayerZeroBridge::constructor: Token cannot be the zero address.");
      require(dailyLimit > 0, "LayerZeroBridge::constructor: Daily limit must be greater than zero.");
   ...
   */
  function addSupportedToken(address _token, uint256 _dailyLimit) external onlyOwner {
    IOFT token = IOFT(_token);

    require(token != IOFT(address(0)), "LayerZeroBridge::addSupportedToken: Token cannot be the zero address.");
    require(_dailyLimit > 0, "LayerZeroBridge::addSupportedToken: Daily limit must be greater than zero.");
    require(supportedTokens[token] == false, "LayerZeroBridge::addSupportedToken: Token is already supported by the bridge.");
  ...
  function removeSupportedToken(address _token) external onlyOwner {
    ...
    require(token != IOFT(address(0)), "LayerZeroBridge::removeSupportedToken: Token cannot be the zero address.");
    require(supportedTokens[token] == true, "LayerZeroBridge::removeSupportedToken: Token is not supported by the bridge.");
   ...
   */
  function updateDailyLimit(address _token, uint256 _newLimit) external onlyOwner {
    IOFT token = IOFT(_token);

    require(token != IOFT(address(0)), "LayerZeroBridge::updateDailyLimit: Token cannot be the zero address.");
    require(supportedTokens[token] == true, "LayerZeroBridge::updateDailyLimit: Token is not supported by the bridge.");
    require(_newLimit > 0, "LayerZeroBridge::updateDailyLimit: New daily limit must be greater than zero.");
  ...
  function unpause() external onlyOwner {
     ...
     */ 
  function bridge(address _token, address_recipient, uint16 _dstChainId, uint256 _amount) external payable whenNotPaused {
    IOFT token = IOFT(_token);
  
    require(supportedTokens[token] == true, "LayerZeroBridge::bridge: Token is not supported by the bridge.");
    require(_recipient != address(0), "LayerZeroBridge::bridge: Recipient cannot be the zero address.");
    require(_dstChainId != 0, "LayerZeroBridge::bridge: Chain ID cannot be zero.");
    require(_amount > 0, "LayerZeroBridge::bridge: Cannot transfer zero tokens.");
    require(token.allowance(msg.sender, address(this)) >= _amount, "LayerZeroBridge::bridge: Token allowance is less than the amount to be transferred.");
  
    token.sendFrom{value: msg.value}(msg.sender, _dstChainId, abi.encodePacked(_recipient), _amount, payable(msg.sender), address(0), bytes(""));
  
    emit BridgeTransfer(token, msg.sender, _recipient, _dstChainId, _amount, block.timestamp);
  }
  ...
  function estimateSendFee(
    ...
    (uint256 nativeFee, uint256 zroFee) = token.estimateSendFee(_dstChainId, recipient, _amount, _useZro, _adapterParams);
  }
}
